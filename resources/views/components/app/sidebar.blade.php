        ...
        <div class="space-y-8">
            ...
            <div>
                <h3 class="text-xs uppercase text-gray-400 dark:text-gray-500 font-semibold pl-3">
                    ...
                    <span class="lg:hidden lg:sidebar-expanded:block 2xl:block">Chats</span>
                </h3>
                @php
                    $user = Auth::user();
                    $chats = $user ? $user->chats()->orderByDesc('updated_at')->limit(10)->get() : collect();
                @endphp
                @if($chats->count())
                    <ul class="mt-2 mb-4">
                        @foreach($chats as $chat)
                            <li class="mb-1 last:mb-0">
                                <a href="{{ route('chatbot', ['chatId' => $chat->id]) }}" class="block px-3 py-2 rounded-lg text-sm truncate bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 hover:bg-blue-100 dark:hover:bg-blue-900 transition">
                                    {{ $chat->title ?? 'Chat #' . $chat->id }}
                                    <span class="text-xs text-gray-400 ml-2">{{ $chat->updated_at->format('d/m H:i') }}</span>
                                </a>
                            </li>
                        @endforeach
                    </ul>
